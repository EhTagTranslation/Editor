...
import { FormControl, Validators, FormGroup, AbstractControl, type ValidationErrors } from '@angular/forms';
...
import { suggestTag, type Tag as TagSuggest } from '#shared/ehentai';
import { namespaceMapToSearch, parseTag, tagAbbrFull } from '#shared/tag';
import { type Tag, NamespaceName, type FrontMatters } from '#shared/interfaces/ehtag';
import { snackBarConfig } from '#browser/environments/environment';
import { editableNs, type ETKey } from '#browser/interfaces/ehtranslation';
...
export class EditorComponent implements OnInit {
    ...
    async submit(): Promise<void> {
        ...
        try {
            ...
            const message = (httpError.error as { message: string })?.message ?? httpError.message ?? String(ex);
            this.snackBar
                .open(`提交过程中出现错误：${message}`, '重试', snackBarConfig)
                .onAction()
                .subscribe(() => {
                    void this.submit();
                });
